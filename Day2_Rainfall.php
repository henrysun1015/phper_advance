...
{
    $pattern1 = "/".$town.":([A-Z][a-z]{2}\s(\d+.\d+)[,]?)*/";
    $pattern2 = "/([A-Z][a-z]{2}\s(\d+.\d+)[,]?)/";
    preg_match($pattern1, $strng, $matches1);
    preg_match_all($pattern2, $matches1[0], $matches);
    if(empty($matches[2])){
    	return -1;
    }
    var_dump($matches[2]);
    return array_sum($matches[2])/count($matches[2]);
}
function variance($town, $strng)
{
    ...
    var_dump($matches[2]);
    $average = array_sum($matches[2])/count($matches[2]);
    $variance_sum = $count = 0;
    foreach ($matches[2] as $key => $value) {
    	$count++;
    	$variance_sum+= ($average-$value)*($average-$value);
    }
    return $variance_sum/$count;
}
var_dump(mean('Lima',$data2));
var_dump(variance('Lima',$data2));
exit;
